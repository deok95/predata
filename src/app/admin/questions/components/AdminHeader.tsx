    ...
    <div className="mb-8 flex items-center justify-between">
      <div>
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className={`text-3xl font-black ${isDark ? 'text-white' : 'text-slate-900'}`}>Question Management</h1>
        </div>
        <p className={isDark ? 'text-slate-400' : 'text-slate-600'}>Create and manage prediction market questions</p>
      </div>
      <button
      ...
      >
        <Plus size={20} />
        {showCreateForm ? 'Close Form' : 'Create New Question'}
      </button>
    </div>
  );
}
