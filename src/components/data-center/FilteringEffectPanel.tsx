  ...
  return (
    <div className={cardClass}>
      <div className="flex items-center gap-2 mb-4">
        ...
        <h2 className={`text-lg font-black ${isDark ? 'text-white' : 'text-slate-900'}`}>어뷰징 필터링 효과</h2>
      </div>
      <p className="text-xs text-slate-400 mb-5">무지성 투표(2초 이하 응답)를 필터링하여 데이터 품질을 향상시킵니다.</p>
      ...
      <div className="flex items-center justify-between mt-2 text-xs">
        <span className="font-bold text-emerald-500">유효 {remaining.toLocaleString()}</span>
        <span className="font-bold text-rose-500">제거 {filtered.toLocaleString()} ({filteringEffect.filteredPercentage.toFixed(1)}%)</span>
      </div>
    </div>
  );
}
