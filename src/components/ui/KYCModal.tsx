...
export default function KYCModal({ isOpen, onClose, onKYCComplete }: KYCModalProps) {
  ...
  const validateForm = () => {
    ...
    if (!kycData.email.includes('@')) newErrors.email = 'Please enter a valid email';
    if (!kycData.birthYear || Number(kycData.birthYear) < 1900 || Number(kycData.birthYear) > 2020) newErrors.birthYear = 'Please enter a valid birth year';
    if (!kycData.location) newErrors.location = 'Please select your location';
    if (!kycData.occupation) newErrors.occupation = 'Please select your occupation';
    setErrors(newErrors);
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
      <div className={`relative w-full max-w-lg rounded-3xl border p-8 shadow-2xl animate-slide-up ${isDark ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-100'}`}>
        ...
        {kycStep === 1 && (
          <>
            <div className="text-center mb-8">
              ...
              <h3 className={`text-2xl font-black mb-2 ${isDark ? 'text-white' : 'text-slate-900'}`}>Level 2 Ticketer Verification</h3>
              <p className="text-sm text-slate-400">Get higher rewards and expert treatment</p>
            </div>
            <div className="space-y-4 mb-8">
              <div className={`flex items-center space-x-4 p-4 rounded-2xl ${isDark ? 'bg-slate-800' : 'bg-slate-50'}`}>
                ...
                <div>
                  <p className="font-bold text-sm">Level 1 (Current)</p>
                  <p className="text-xs text-slate-400">Reward Multiplier 1%</p>
                </div>
              </div>
              ...
              <div className={`flex items-center space-x-4 p-4 rounded-2xl border-2 border-indigo-600 ${isDark ? 'bg-indigo-950/20' : 'bg-indigo-50'}`}>
                ...
                <div>
                  <p className="font-bold text-sm">Level 2 (After Verification)</p>
                  <p className="text-xs text-slate-400">Reward Multiplier 50%</p>
                </div>
              </div>
            </div>
        ...
        {kycStep === 2 && (
          <>
            <h3 className={`text-xl font-black mb-6 ${isDark ? 'text-white' : 'text-slate-900'}`}>Enter Basic Information</h3>
            <div className="space-y-4 mb-8">
              <div>
              ...
              <div>
                <label className="text-xs font-bold text-slate-400 uppercase mb-2 block">Birth Year</label>
              ...
              <div>
                <label className="text-xs font-bold text-slate-400 uppercase mb-2 block">Occupation</label>
        ...
        {kycStep === 3 && (
          <div className="text-center py-12">
            ...
            <h3 className={`text-2xl font-black mb-2 ${isDark ? 'text-white' : 'text-slate-900'}`}>Verification Complete!</h3>
            <p className="text-slate-400 mb-4">Promoted to Level 2 Ticketer</p>
            <div className={`rounded-2xl p-4 inline-block ${isDark ? 'bg-emerald-950/20' : 'bg-emerald-50'}`}>
              <p className="text-sm font-bold text-emerald-600">Reward Multiplier: 1% → 50%</p>
            </div>
          </div>
        )}
      </div>
