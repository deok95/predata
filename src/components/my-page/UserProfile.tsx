    ...
    <div className={`p-6 rounded-3xl border ${isDark ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-100'}`}>
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <span className="text-sm text-slate-400 flex items-center"><Wallet size={14} className="mr-2" /> Wallet</span>
          {user.walletAddress ? (
        ...
        <div className="flex items-center justify-between">
          ...
          <span className="text-sm font-bold">{user.totalPredictions ?? 0}</span>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center animate-fade-in">
          ...
          <div className={`relative w-full max-w-md rounded-3xl border p-8 shadow-2xl ${isDark ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-100'}`}>
            <h3 className={`text-xl font-black mb-4 ${isDark ? 'text-white' : 'text-slate-900'}`}>
              {user.walletAddress ? 'Change Wallet Address' : 'Connect Wallet'}
            </h3>
            <p className="text-sm text-slate-400 mb-6">
              {user.walletAddress
                ? 'Connecting a new wallet will disconnect the previous wallet.'
                : 'Please connect your wallet for deposits/withdrawals.'}
            </p>
            ...
            {!isConnected ? (
              ...
              <div className="space-y-4">
                <div className={`p-4 rounded-xl ${isDark ? 'bg-slate-800' : 'bg-slate-50'}`}>
                  <p className="text-xs text-slate-400 mb-2">Wallet Address to Connect</p>
